// encounter-plains-cold.js (edit)
export async function execute() {
    console.log("Executing cold plains encounter...");
    ...
    const encounterTables = {
      "very rare": [
        ...
        "Ice Hag (MCB); night", "Owlbear, Arctic (MM)", "Unicorn, Black (MCB); night", "Roll on Table 15.5: Extremely rare monsters"
      ],
      "rare": [
        ...
        "Human, Mercenary (MM)", "Lycanthrope, Werebear (MM); night", "Peryton (MM)", "Roll on Table 15.4: Very rare monsters"
      ],
      "uncommon": [
        ...
        "Ruve (FF)", "Aranea (MCC)", "Horse, Wild (MM); day", "Human, Police/Constabulary (MM)",
        "Wolf (MM)", "Xvart (FF)", "Cat, Great, Snow Tiger (MCC); day"
      ],
      "common": [
        "Bat, common (MCA); night", "Camel (MCA)", "Cattle (MCA)", "Sheep (MCA)",
        "Beetle, giant, boring (MM); night", "Broken one, common (MM); night", "Centipede, giant (MM)", "Centipede, huge (MM)",
        "Ore (MM)", "Wild dog (MM)", "Human, Peasant/Serf (MM)", "Human, Slaver (MM)",
        "Lycanthrope, Werewolf (MM); night", "Ogre (MM)", "Rat, Giant (MM); night", "Human, Bandit/Brigand (MM)",
        "Human, Farmer/Herder (MM)", "Human, Gentry (MM)", "Human, Merchant/Trader (MM)", "Human, Middle Class (MM)",
        "Human, Thief/Thug (MM)", "Human, Tradesman/Craftsman (MM)", "Mammal, Wild Boar (MM)", "Mammal, Goat (MM)",
        "Mammal, Rothe (MM)", "Mammal, Bull (Wild Ox) (MCB)", "Mammal, Caribou (MCB)"
      ]
    };
    ...
    const resultMessage = `<strong>Cold Plains Encounter:</strong> ${encounterResult}`;
    game.recentEncounterResult = resultMessage;
  
    ChatMessage.create({
      speaker: { alias: "Cold Plains Adventure" },
      content: `<h2>Cold Plains Encounter</h2><p>${resultMessage}</p>`
    });
  }
