// encounter-cold-subterranean.js (edit)
export async function execute() {
    console.log("Executing cold subterranean encounter...");
    ...
    const encounterTables = {
        ...
        "rare": [
            ...
            "Human, Mercenary (MM)", "Lycanthrope, Werebear (MM); night", "Peryton (MM)", "Roll on Table 18.4: Very rare monsters"
        ],
        "uncommon": [
            ...
            "Snake, Poison, Normal (MM)", "Snake, Poison, Giant (MM)", "Troll (MM); night", "Wight (MM); night",
            "Ruve (FF)", "Aranea (MCC)", "Bugbear (MM)", "Giant, Fomorian (MM)",
            "Gnoll (MM); night", "Goblin (MM); night", "Gremlin, Jermlaine (MM)", "Lizard, Subterranean; day (MM)",
            "Wolf (MM)", "Xvart (FF)", "Whipsting (MCA)", "Giant Toad, Poisonous (MM)",
            "Wall-Walker (MCB)", "Varkha (MCD)"
        ],
        "common": [
            "Bat, common (MCA); night", "Beetle, giant, boring (MM); night", "Beetle, giant, fire (MM); night", "Broken one, common (MM); night",
            "Centipede, giant (MM)", "Centipede, huge (MM)", "Ore (MM)", "Lycanthrope, Werewolf (MM); night",
            "Ogre (MM)", "Rat, Giant (MM); night", "Fungus, Shrieker (MM)", "Troglodyte (MM)"
        ]
    };
    ...
    const resultMessage = `<strong>Cold Subterranean Encounter:</strong> ${encounterResult}`;
    game.recentEncounterResult = resultMessage;

    ChatMessage.create({
        speaker: { alias: "Cold Subterranean Adventure" },
        content: `<h2>Cold Subterranean Encounter</h2><p>${resultMessage}</p>`
    });
}
