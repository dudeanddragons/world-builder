 ...
 */
    ...
    async createJournalEntry() {
      ...
      const storyContent = wbData.story
          ? wbData.story
                .map((storyItem, index) => {
                    const titleContent = storyItem.title
                        ? `<h1>${storyItem.title}</h1>` // Add header only if title is not blank
                        : "";
                    return `
                        ${titleContent}
                        <p>${storyItem.description || "No description provided."}</p>
                    `;
                })
                .join("")
          : "<p>No story details provided.</p>";
      ...
      const journalContent = {
          ...
          pages: [
              ...
              {
                  name: wbData.storyTitle || "Unnamed Story", // Use storyTitle as page name
                  type: "text",
                  text: {
                      content: `
                        ${
                            wbData.storyImage
                                ? `<img src="${wbData.storyImage}" alt="Story Image" style="max-width: 100%; height: auto;" />`
