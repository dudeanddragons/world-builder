// encounter-swamp-cold.js (edit)
export async function execute() {
    console.log("Executing cold swamp encounter...");
    ...
    const encounterTables = {
        "very rare": [
            ...
            "Darkenbeast (FR2); night", "Elemental Vermin, Air (Duster) (MCA)", "Elemental Vermin, Earth (Crawler) (MCA)",
            "Flameskull (MCA)", "Gulguthydra (MCB)", "Roll on Table 21.5: Extremely rare monsters"
        ],
        "rare": [
            "Bat, azmyth (MM)", "Bat, huge (mobat) (MCA); night", "Bat, sinister (MM)", "Behir (MM); day",
            "Ghoul, Ghast (MM); night", "Human, Barbarian/Nomad (MM)", "Lycanthrope, Wereboar (MM)", "Medusa (MM)",
            "Medusa, Greater (MM)", "Mongrelman (MM); night", "Ogre, Ogrellon (MM)", "Plant, Dangerous, Thorn-Slinger (MM)",
            ...
            "Sull (FR2)", "Hellcat (FF)", "Penanggalan (FF); night", "Baneguard (MCA)",
            ...
            "Chosen One (MCC)", "Dread Warrior (MCC)", "Black dragon (MM)", "Hydra, Pyrohydra (MM)",
            "Troll, Snow (MCA); night", "Roll on Table 21.4: Very rare monsters"
        ],
        "uncommon": [
            ...
            "Ruve (FF)", "Aranea (MCC)", "Frog, Giant (MM)", "Hydra (MM)",
            "Will o'wisp (MM); night", "Roll on Table 21.3: Rare monsters"
        ],
        "common": [
    ...
    const resultMessage = `<strong>Cold Swamp Encounter:</strong> ${encounterResult}`;
    game.recentEncounterResult = resultMessage;

    ChatMessage.create({
        speaker: { alias: "Cold Swamp Adventure" },
        content: `<h2>Cold Swamp Encounter</h2><p>${resultMessage}</p>`
    });
}
