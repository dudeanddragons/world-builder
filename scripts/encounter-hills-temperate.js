...
export async function execute() {
    ...
    const encounterTables = {
      "very rare": [
        ...
        "Flumph, Monastic (CM2); night", "Owlbear, Arctic (MM)", "Unicorn, Black (MCB); night",
        "Roll on Table 8.5: Extremely rare monsters"
      ],
      "rare": [
        ...
        "Roll on Table 8.4: Very rare monsters"
      ],
      "uncommon": [
        ...
        "Hobgoblin (MM)", "Goblin (MM); night", "Hobgoblin (MM)", "Horse, Wild (MM); day",
        "Human, Police/Constabulary (MM)", "Mammal, Porcupine (MM)", "Bird, Owl (MM)", "Mammal, Skunk (MM)"
      ],
      "common": [
        ...
        "Bear, black (MM); day", "Dwarf, Hill (MM)", "Human, Bandit/Brigand (MM)", "Human, Farmer/Herder (MM)"
      ]
    };
    ...
    const resultMessage = `<strong>Temperate Hills Encounter:</strong> ${encounterResult}`;
    game.recentEncounterResult = resultMessage;
  
    ChatMessage.create({
      speaker: { alias: "Temperate Hills Adventure" },
      content: `<h2>Temperate Hills Encounter</h2><p>${resultMessage}</p>`
    });
  }
