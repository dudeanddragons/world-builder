...
export async function execute() {
    ...
    const encounterTables = {
        ...
        "rare": [
            ...
            "Human, Mercenary (MM)", "Lycanthrope, Werebear (MM); night", "Peryton (MM)", "Roll on Table 16.4: Very rare monsters"
        ],
        "uncommon": [
    ...
    const resultMessage = `<strong>Warm Subterranean Encounter:</strong> ${encounterResult}`;
    game.recentEncounterResult = resultMessage;

    ChatMessage.create({
        speaker: { alias: "Warm Subterranean Adventure" },
        content: `<h2>Warm Subterranean Encounter</h2><p>${resultMessage}</p>`
    });
}
